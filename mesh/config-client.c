				...
				uint16_t len, void *user_data)
{
	...
	case OP_CONFIG_MODEL_SUB_STATUS:
		...
		if (data[0] != MESH_STATUS_SUCCESS) {
			config_emit_new_status("sub", -data[0],
				"node=%q",
				"Node %4.4x Subscription status %s",
				src,
				src, mesh_status_str(data[0]));
	...
	case OP_CONFIG_MODEL_SUB_LIST:
		...
		config_emit_new_status("sub_list", -data[0],
	...
	case OP_MODEL_APP_LIST:
		...
		config_emit_new_status("mod_appidx", -data[0],
	...
	case OP_NODE_RESET_STATUS:
		...
		config_emit_new_status("node", -data[0],
				"node=%q",
...
static const struct bt_shell_menu cfg_menu = {
	...
	{"composition_get",	"[page_num]",		cmd_composition_get,
						"Get composition data"},
	{"netkey_add",		"<net_idx>",		cmd_netkey_add,
						"Add network key"},
	{"netkey_del",		"<net_idx>",		cmd_netkey_del,
						"Delete network key"},
	{"appkey_add",		"<app_idx>",		cmd_appkey_add,
						"Add application key"},
	{"appkey_del",		"<app_idx>",		cmd_appkey_del,
						"Delete application key"},
	{"bind",		"<ele_idx> <app_idx> <mod_id> [cid]",
				cmd_bind,	"Bind app key to a model"},
	{"mod_appidx_get", "<ele_addr> <model id>",
				cmd_mod_appidx_get,    "Get model app_idx"},
	{"ttl_set",		"<ttl>",		cmd_ttl_set,
						"Set default TTL"},
	{"ttl_get",		NULL,			cmd_ttl_get,
						"Get default TTL"},
	{"pub_set", "<ele_addr> <pub_addr> <app_idx> "
			"<per (step|res)> <re-xmt (cnt|per)> <mod id> "
			"[cid]",
			cmd_pub_set,	"\n\t\t\t\t\t\t  Set publication"},
	{"pub_get", "<ele_addr> <model>",               cmd_pub_get,
						"Get publication"},
	{"proxy_set",           "<proxy>",              cmd_proxy_set,
						"Set proxy state"},
	{"proxy_get",           NULL,                   cmd_proxy_get,
						"Get proxy state"},
	{"ident_set",           "<net_idx> <state>",    cmd_ident_set,
						"Set node identity state"},
	{"ident_get",           "<net_idx>",            cmd_ident_get,
						"Get node identity state"},
	{"beacon_set",           "<state>",             cmd_beacon_set,
						"Set node identity state"},
	{"beacon_get",           NULL,                  cmd_beacon_get,
						"Get node beacon state"},
	{"relay_set",           "<relay> <rexmt count> <rexmt steps>",
						cmd_relay_set,
						"Set relay"},
	{"relay_get",           NULL,                   cmd_relay_get,
						"Get relay"},
	{"hb_pub_set", "<pub_addr> <count> <period> <ttl> <features> <net_idx>",
				cmd_hb_pub_set,	"Set heartbeat publish"},
	{"hb_pub_get",           NULL,                   cmd_hb_pub_get,
						"Get heartbeat publish"},
	{"hb_sub_set", "<src_addr> <dst_addr> <period>",
				cmd_hb_sub_set,     "Set heartbeat subscribe"},
	{"hb_sub_get",           NULL,                   cmd_hb_sub_get,
						"Get heartbeat subscribe"},
	{"sub_add", "<ele_addr> <sub_addr> <model id>",
				cmd_sub_add,    "Add subscription"},
	{"sub_list_get", "<ele_addr> <model id>",
				cmd_sub_get,    "Get subscription"},
	{"node_reset",		NULL,                    cmd_node_reset,
